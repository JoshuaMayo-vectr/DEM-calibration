...
| 13 | Cohesion model (SJKR) | ✅ (qualified — plumbing shipped: a cohesive material (`cohesion: "sjkr"`) renders `cohesion sjkr` + a calibratable `cohed` (cohesionEnergyDensity) in its own cache namespace, opt-in search dimension, smoke-verified end-to-end on a real cohesive LIGGGHTS heap; calibration deferred — wheat is cohesionless and no cohesion-sensitive measured target exists, per the identifiability discipline) |
| 14 | Configurable contact model + protocol/geometry | ✅ (qualified — plumbing shipped: contact model (`normal_model` hertz/hooke, `rolling_model` cdt/epsd/epsd2/epsd3) and heap geometry (`cyl_radius_m`/`cyl_height_m`) are config-selectable in their own cache namespace, byte-identical default render preserved, smoke-verified on a real epsd + R 0.050 m heap; protocol *speeds* stayed code-level by design, and drum/orifice geometry was deferred — only the heap was wired/demonstrated per the identifiability discipline) |
| ⚑  | **Checkpoint 5 — Configurable physics sufficient, or is shape required?** | ⬜ (gated on a target material that exercises the new knobs against discriminating data — same dependency as 12–13) |
| 15 | Particle shape (multisphere) | ✅ (qualified — plumbing shipped: a `particle_shape: "multisphere"` material renders `particletemplate/multisphere` + the `fix multisphere` integrator + a `mol` body-id dump column in its own cache namespace, byte-identical single-sphere default preserved, heap-only scope (drum/drum45 pinned single-sphere), opt-in, smoke-verified end-to-end on a real LIGGGHTS clump heap (AoR **25.4° → 37.3°** vs the single-sphere twin at the same friction — the shape lever single spheres can't reach); calibration deferred — no measured wheat shape target, and shape stays a model-form selector not a search dimension, per the identifiability discipline) |
...
| **5** | Phase 14 | **Do the configurable parameter extensions match the new material's responses within spread — or is particle shape (model form) required?** Independent wall friction, cohesion, and contact-model choice, calibrated against discriminating physical data, reproduce the new material's bulk responses. *(Phase 15 now ships multisphere shape as capability too — the smoke heap shows shape reaches AoR 37.3° where the single sphere tops out at 25.4° on the same friction, so the model-form lever exists; whether it is **required** still waits on a target material with discriminating shape-sensitive data.)* | Invest in multisphere shape (**Phase 15 — done as plumbing**); or if faceted/angular shape is first-order, escalate to a polyhedra- and GPU-capable engine (Aspherix / Rocky / EDEM) — the architecture survives the swap (only `templates/` + dump parser change). |
...
_Phases 0–11 build and validate the pipeline on a single proof-of-concept material (dry, cohesionless, single-sphere). Phases 12–15 are the **physics-extension track** that lifts the assumptions locked during that proof-of-concept so the pipeline can calibrate arbitrary granular materials — **all four (12 independent wall friction, 13 SJKR cohesion, 14 configurable contact model + heap geometry, 15 multisphere particle shape) are now shipped as plumbing** (2026-06-13), each smoke-verified on a real LIGGGHTS run with calibration deferred to a discriminating measured target. They share one architectural truth — the templated `-var` inputs and the cache-namespaced `params_hash` (Phase 8.5) already anticipate new parameters, so each is plumbing into existing structure, not surgery (Phase 12 confirmed this — the change was config/optimizer/UI wiring, with the templates and cache untouched; Phase 13 did touch the templates — SJKR is genuinely new contact physics — but only behind a conditional that leaves the cohesionless default render byte-identical and the wheat cache untouched). And they share one discipline: a new calibrated dimension is worth adding **only if a discriminating physical measurement exists to constrain it** — otherwise it widens the equivalence family rather than localizing a point (the Phase-9 lesson). Both Phase 12 (wall friction) and Phase 13 (cohesion) honoured that discipline by shipping the *capability* while explicitly deferring the *calibration* to a measured target — a wheat–wall friction pair and a cohesion-sensitive bench measurement, respectively._
...
**Lessons learned.** *(plumbing + smoke completed 2026-06-13 — templates `aor.in.j2`/`drum.in.j2`/`drum45.in.j2`, `runner.py`/`optimize.py`/`ui.py`/`materials/schema.json` wiring, tests in `tests/test_runner*.py` + `tests/test_optimize.py`, full record in [results/phase13-cohesion/NOTES.md](results/phase13-cohesion/NOTES.md))*

- **The capability ships; the demonstration does not** — same discipline as Phase 12. Wheat is cohesionless and no cohesion-sensitive measured target exists, so calibrating `cohed` would only widen the equivalence family. The exit criterion's "a cohesion-sensitive response is matched within spread" is deliberately deferred to a measured target; the remaining work is then mechanical (cohesive card → add a cohesion-sensitive response + its noise floor → name `cohed` in `search_bounds` → re-run → verify at 5 seeds).
- **Unlike Phase 12, this WAS new template physics** — but behind a material-gated jinja conditional (`{% if cohesive %}` + `tangential history{{ cohstr }} ...`) with `trim_blocks`/`lstrip_blocks`, so the cohesionless default render stays **byte-identical** to the static `.in` (the Phase-8.5 contract held) and the Phase 7–10 wheat cache is untouched. `cohed` joins the canonical hash **only when > 0**; a cohesive material gets its own namespace via the material hash. Verified: cohesive run namespace `5182aa437c`, wheat candidate still an instant hit on the pinned `a3338ce730`.
- **SJKR works end-to-end on a real heap:** cohed 12000 J/m³ → AoR 28.19° vs the cohesionless twin's 25.42° (same PSD/density/E) — **+2.8°, the correct direction** (cohesion holds a steeper heap); rendered + measured cleanly, ~232 s/run.
- **Keyword ORDER bit us:** LIGGGHTS rejects `cohesion` placed after `rolling_friction` (*"wrong keyword order"*, `pair_gran_base.h:129`) — the model parser walks a fixed sequence, so cohesion must sit right after `tangential history`. The knob catalog had the order backwards and was corrected.
- **The hold-out is double-pinned cohesionless** — `drum45` drops any passed `cohed` (canonical) AND never renders SJKR (`_render_text`), the same defense as the unconditional `fricpw` pin: a cohesive `best.json` must not be able to un-pin the wheat validation.
- **Wall limit ×3 for SJKR** (Phase-1's runtime warning) as a safety margin, though this `cohed` actually ran at ~vertical-heap cost.
...
**Lessons learned.** *(plumbing + smoke completed 2026-06-13 — templates `aor.in.j2`/`drum.in.j2`/`drum45.in.j2`, `runner.py`/`optimize.py`/`ui.py`/`materials/schema.json` wiring, tests in `tests/test_runner_phase14.py` + extended `tests/test_measure.py`/`tests/test_optimize.py`)*

- **The capability ships; the demonstration is the heap** — same discipline as Phases 12–13, and the scope was deliberately bounded to it (user-confirmed): contact model fully wired across all three templates, geometry made config-driven but demonstrated and synthetic-tested on the **heap (cylinder)** only — the least measurement-coupled rig. **Drum/orifice geometry is deferred** (their meshes/regions/measurement windows are tightly coupled and would re-open the Phase-9/10 calibrations); `_mesh_for` returns the static drum mesh for any non-`aor` response, and only `cyl_radius_m`/`cyl_height_m` are exposed.
- **Exit criterion met end-to-end.** A real heap with a **non-epsd2 rolling model (epsd) AND a non-default cylinder (R 0.050 / H 0.120)** selected from config alone simulated, measured (AoR 26.24°, 1 seed), and landed in its own cache namespace `6df9c42bbd` — distinct from the wheat baseline `a3338ce730`, which is **unchanged** (the pinned-hash regression still passes). LIGGGHTS accepted the `rolling_friction epsd` model string + the `coefficientRollingViscousDamping` fix + the generated `cylinder_r0.05_h0.12.stl` on the first attempt.
- **Contact model lives in the material namespace, geometry in a sibling `geometry` config block — both fold into one validated dict for the runner.** This kept the runner threading to almost nothing: `material_canon` validates/canonicalizes the new keys, `_MAT_HASH_KEYS`/`_WHEAT_CANON_CORE` gained them at their locked defaults (hertz/epsd2/0.040/0.100), so a material spelling out the defaults still canonicalizes to `None` → byte-identical static render, legacy cache preserved. Config schema bumped **v2 → v3** (v1/v2 load with locked-default geometry).
- **`epsd`/`epsd3` were genuinely new template physics** (a `coefficientRollingViscousDamping` `fix`), `hooke` likewise (a `characteristicVelocity scalar` fix) — both behind material-gated `{% if %}` conditionals with `trim_blocks`/`lstrip_blocks`, so the hertz/epsd2 default render stays byte-identical (the Phase-8.5/13 contract held for the third time). **Keyword order** from the Phase-13 correction still holds: `tangential history [cohesion] rolling_friction <variant>`.
- **`rollvisc` follows the cohed opt-in contract exactly**: a searchable dimension (`SEARCHABLE_DIMS`) that enters `canonical`/the hash **only when > 0**, gated at config load to epsd/epsd3 materials (mirrors the cohed-needs-SJKR guard). μ_r is **not portable across rolling variants** (knob-catalog warning) — the card records the variant, and switching it is a recalibration, not a tweak.
- **The 45° hold-out is triple-pinned now** — `drum45` drops `cohed` AND `rollvisc` in `canonical` and renders hertz/epsd2 unconditionally in `_render_text`, the same defense as the unconditional `fricpw` pin: a non-default `best.json` (cohesive, epsd, or hooke) must never un-pin the validated validation.
- **Geometry follows through measurement and camera, not just the mesh**: the insertion region and safety walls scale in the template (`_aor_region_ctx`, byte-identical at the default), `measure_bulk_density(cyl_radius=…)` and the snapshot `fov` thread from the runner, and the heap **angle fit needed no change** — it is data-driven (centroid + median-diameter bins), which is exactly why the heap was the clean demonstration target (new synthetic-cone test passes at ±0.5° on a wider rig).
- **A footgun to record:** leaving the heap particle count at the wheat default (4000) in a *larger* cylinder gives a shorter/wider fill and the bulk-density slab/total cross-check warns (`> 12%` disagreement) — the AoR still measures fine, but `n_particles` should scale with a bigger rig. The UI's insertion-capacity warning (now geometry-aware via `_aor_insert_volume`) surfaces the under-fill; the density warning is the measurement's own backstop.
- **Protocol *speeds* stayed code-level by design** (user-confirmed): `LIFTV` (10 mm/s) and `ROTPER` (5 rpm) remain template `-var` defaults, never surfaced in config — lift speed is rate-sensitive (Phase 3) and the meshes/cameras/measurement windows assume the protocol. Only rig *dimensions* are per-study.

→ **Checkpoint 5**: Configurable physics sufficient, or is shape required? — **not yet decidable, by the same gate as Phases 12–13.** The configurable knobs (wall friction, cohesion, contact-model variant, geometry) **and multisphere shape (Phase 15)** are all *shipped and verified as capability* — the shape smoke run even shows the lever reaches AoR 37.3° where the single sphere tops out at 25.4° on the same friction — but whether the configurable knobs *suffice* vs whether shape is *required* can only be answered against **a target material that exercises them with discriminating measured data**. Wheat is single-sphere/cohesionless/default-geometry and matched at the family level already (Phase 9/10), so it cannot test the new knobs. The decision waits on physical bench data on a material whose bulk behaviour the sphere model demonstrably *cannot* reach — the residual model-form bias quantified in [results/phase10-validation/NOTES.md](results/phase10-validation/NOTES.md) is the standing hint that shape may eventually be required.
...
**Lessons learned.** *(plumbing + smoke completed 2026-06-13 — template `aor.in.j2`, `runner.py`/`measure.py`/`optimize.py`/`ui.py`/`materials/schema.json` wiring, tests in `tests/test_runner_phase15.py` + extended `tests/test_measure.py`/`tests/synth.py`/`tests/test_optimize.py`, full record in [results/phase15-shape/NOTES.md](results/phase15-shape/NOTES.md))*

- **The capability ships; the demonstration is the heap** — same discipline and same heap-only bounding as Phase 14. Multisphere is wired into `aor` only; `drum`/`drum45` carry no multisphere block, so the `response == "aor"` render guard pins the drum and the 45° hold-out to single spheres — the **fourth pin**, alongside cohesion / wall-friction / contact-model. Calibration is deferred: no measured wheat shape target exists, and wheat is already family-matched by spheres, so calibrating a clump would only widen the equivalence family.
- **Exit criterion demonstrated end-to-end.** A real prolate 3-sphere wheat clump (aspect ≈ 2.2) inserted, simulated, rendered and measured in its own cache namespace `70190ea252` on the first attempt — LIGGGHTS accepted `particletemplate/multisphere` + the inline `spheres` list + `fix multisphere` with no fight. AoR **25.4° → 37.3°** (single-sphere → clump, same fric 0.50 / μ_r 0.12 / e 0.50): the shape lever reaches a heap angle the single sphere **cannot** at that friction — exactly the model-form residual the Phase-10 45° validation flagged (~−4°). The wheat baseline `a3338ce730` stays a byte-identical instant cache hit.
- **The integrator swap was the one structural change** beyond a template conditional: `fix multisphere` *replaces* `nve/sphere` (both present is a LIGGGHTS error) — unlike Phases 13/14 which only *added* fixes. Still behind a `{% if multisphere %}`/`{% else %}` pair, so the single-sphere default render stays byte-identical (the Phase-8.5 contract held a fourth time).
- **Measurement: angle needs nothing, density needs grouping.** The heap free surface *is* the sub-sphere top envelope, so `measure_angle` reads a clump heap straight from the sub-sphere cloud (synthetic-cone test passes at the same ±0.5° as single spheres). Bulk density does change: summing `(4/3)πr³` over sub-spheres double-counts intra-clump overlap (the naive path over-read 615 vs 559 on a synthetic packing), fixed by grouping on the new `mol` body-id column and weighing each body by `ρ·V_clump`. Our deterministic Monte-Carlo `_clump_equiv_volume` matches LIGGGHTS' own body mass to **0.09%** (7.139e-08 vs 7.145e-08 m³).
- **Shape is a model-form selector, not a search dimension** — like cohesion/normal_model/rolling_model, it lives in the material namespace, not in `canonical(params)`, so no new `-var` is needed (the clump renders into the `.in`). A *continuous* shape knob (aspect-ratio) would be the mechanical follow-up when a shape-discriminating target exists — added to `SEARCHABLE_DIMS` with a `particle_shape == multisphere` guard, mirroring the cohed/rollvisc opt-ins.
- **`all_in yes` under-fills with clumps** (the flagged insertion risk, as it played out): 1250 of 3000 requested clumps fit the region (the bounding sphere is larger) — it did not stall, and the AoR fit cleanly (r² 0.954), but the shallow bed tripped the bulk-density slab-vs-total backstop. A faithful multisphere heap should raise `n_particles` (or relax insertion to `volumefraction_region`). Serial `-np 1` cost 404 s for ~3750 sub-spheres — within the planned ~2–4× single-sphere budget.
...
- **Identifiability of the new calibrated dimensions** *(opened by Phases 12–15)*. Every degree of freedom these phases unlock — wall friction (12), cohesion (13), contact-model variant + heap geometry (14), shape (15) — is constrained only by a response sensitive to it: adding a dimension without a discriminating physical target yields a *wider* equivalence family, not a localized point (the Phase-9 degeneracy lesson, generalized). **Phases 12–15 are now all shipped as capability (2026-06-13)** — the plumbing, cache namespacing, config wiring and byte-identical-default contract are verified for wall friction, cohesion, contact-model/geometry, **and multisphere shape** — so the gating dependency for the whole extension track is no longer pipeline code but **physical bench data on the target material**: a measured wheat–wall friction pair, a cohesion-sensitive bench measurement, a flow-rate target (the Phase-9 valley breaker), or a shape-sensitive response on a material whose bulk behaviour a single sphere cannot reach (the Phase-15 smoke run quantified that single-sphere/clump gap at +11.9° of AoR). Until such data exists, the new knobs are exercised only as smoke tests, never calibrated.
...
├── templates/            ← LIGGGHTS input scripts: static .in (the byte-identical wheat default) + .j2 variants (aor/drum/drum45 — material, cohesion, contact-model & heap-geometry conditionals; aor also carries the Phase-15 multisphere clump + integrator conditional; drawdown.in static) + STL generators (make_*.py, now driven per-study) and pre-built meshes
